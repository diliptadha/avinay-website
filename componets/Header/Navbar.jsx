...
const Navbar = () => {
  ...
  return (
    <>
      <div
        className={`fixed top-0 w-full transition-all duration-300 ease-in-out z-[100] items-center ${isScrolled
      ...
      >
        <div className="mx-auto max-w-[1200px] w-full px-4 sm:px-6 lg:px-8 relative z-[101]">
                ...
                <ul className="flex gap-10 font-semibold text-xl capitalize items-center">
                  {navItems.map((item, index) => (
                    <li key={index}>
                      <Link
                        href={item.path}
                        className={`${pathname === item.path ? "border-b-3 border-[#8b2727] transition-all text-[#8b2727]" : ""
          ...
          {/* Mobile Menu */}
          <div
            className={`lg:hidden transition-all duration-500 ease-in-out overflow-hidden ${isMenuOpen
            ...
            <nav className="pt-2">
              <ul className="flex flex-col gap-2 font-semibold text-lg capitalize">
                {navItems.map((item, index) => (
                  <li
                    key={index}
                    className={`transition-all duration-300 transform ${isMenuOpen
